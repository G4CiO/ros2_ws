...
private:
                        ...
                        const my_robot_interfaces::srv::SetLED::Response::SharedPtr response)
    {
        ...
        if (led_number == 3 && state == "on")
        {
            response->success = true;
            RCLCPP_INFO(this->get_logger(), "Battery status: Emoty");
            return;
        }
        else if (led_number == 3 && state == "off")
        {
            response->success = true;
            RCLCPP_INFO(this->get_logger(), "Battery status: Full");
            return;
        }
        else
        {
            response->success = false;
            RCLCPP_WARN(this->get_logger(), "Please key again...");
            return;
        }
